...
function TabIcon({ focused, color, size, children }: {
  focused: boolean;
  color: string;
  size: number;
  ...
  return (
    <View className={`items-center justify-center w-10 h-10 rounded-xl ${focused ? "bg-brand-400/20" : ""}`}>
      {children}
    </View>
  );
}
...
function FoodIcon({ color }: { color: string }) {
  return (
    <View style={{ width: 20, height: 20, alignItems: "center", justifyContent: "center" }}>
      ...
      <View style={{ position: "absolute", top: 0, width: 2, height: 7, backgroundColor: color, borderRadius: 1 }} />
    </View>
  );
}
...
function WorkoutIcon({ color }: { color: string }) {
  return (
    <View style={{ width: 22, height: 14, flexDirection: "row", alignItems: "center", gap: 2 }}>
      <View style={{ width: 5, height: 14, borderRadius: 2.5, borderWidth: 2, borderColor: color }} />
...
function ProfileIcon({ color }: { color: string }) {
  return (
    <View style={{ width: 20, height: 20, alignItems: "center" }}>
      ...
      <View style={{ width: 16, height: 7, borderTopLeftRadius: 8, borderTopRightRadius: 8, borderWidth: 2, borderColor: color, marginTop: 1, borderBottomWidth: 0 }} />
    </View>
  );
}
